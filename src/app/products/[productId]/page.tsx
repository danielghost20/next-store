...
export default async function ProductPage({ params }: { params: { productId: number } }) {
    ...
    const productsByCategory = getProductsByCategory(product.category.id)
    return (
        <>
            ...
            <main className="m-auto mt-10 max-w-screen-2xl">
                <div className="flex justify-center w-full min-h-screen gap-10 px-3">
                    ...
                    <div className="flex flex-col max-w-lg gap-5">
                        <h2 className="text-3xl font-semibold">Titulo del Producto</h2>
                        <p className="text-lg">Lorem ipsum dolor sit amet consectetur adipisicing elit. Dignissimos quos corrupti, est ratione, delectus quod ab quis, voluptas ipsum praesentium animi quisquam tempore? Eaque beatae in aperiam, fugiat veritatis sint.</p>
                        <p className="text-lg">Price: ${product.price} MNX</p>
                        <p className="text-lg">Categoria: {product.category.name}</p>
                        <div className="flex gap-5">
                            <button className={`${buttonVariants()} flex gap-2 max-w-max`}>Agregar al <LiaShoppingCartSolid className="text-lg" /></button>
                            <Link href='/' className={`${buttonVariants()} bg-background flex gap-2 text-foreground`}>
                ...
                <section className="flex justify-between w-full px-4">
                    {
                        (await productsByCategory).map((product: Products) => (
                            <ProductOfert cardStyles="w-[340px] h-[400px] border-2 p-4  rounded-md" description={product.description} id={product.id} image={product.images[0]} price={product.price} productName={product.title} key={product.id} />
                        ))
                    }
                </section>
            </main>
